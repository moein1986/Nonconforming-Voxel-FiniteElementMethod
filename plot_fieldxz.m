...
function plot_fieldxz(xmin,zmin,xmax,zmax,y0,A,OT)



x=xmin:(xmax-xmin)/50/pi:xmax;
z=zmin:(xmax-xmin)/50/pi:zmax;
...
ESample_x(iX,iZ)=E1*N1+E2*N2+E3*N3+E4*N4;
ESample_y(iX,iZ)=E5*N5+E6*N6+E7*N7+E8*N8;
ESample_z(iX,iZ)=E9*N9+E10*N10+E11*N11+E12*N12;
    end
end
    set(figure, 'color', 'white');
imagesc(x(:,1), z(1,:), (abs(ESample_x))'); 
title('|E_x| (V/m) x-z plane')

set(figure, 'color', 'white');
imagesc(x(:,1), z(1,:), (abs(ESample_y))');
title('|E_y| (V/m) x-z plane')

set(figure, 'color', 'white');
imagesc(x(:,1), z(1,:), (abs(ESample_z))');
title('|E_z| (V/m) x-z plane')
end
