function [x,flag,relres,iter,resvec] = MYpcgComplex(A,b,tol,maxit,M1,M2,x0,varargin)
...
%   $Revision: 1.18.4.11 $ $Date: 2010/11/17 11:29:49 $
...
[atype,afun,afcnstr] = MYiterchk(A);
...
if tol <= eps
    warning('MATLAB:pcg:tooSmallTolerance', ...
        'Input tol is smaller than eps and may not be achieved by PCG\n         Try to use a bigger tolerance');
    warned = 1;
    tol = eps;
elseif tol >= 1
...
if ((nargin >= 5) && ~isempty(M1))
    existM1 = 1;
    [m1type,m1fun,m1fcnstr] = MYiterchk(M1);
...
if ((nargin >= 6) && ~isempty(M2))
    existM2 = 1;
    [m2type,m2fun,m2fcnstr] = MYiterchk(M2);
...
r = b - MYiterappComplex('mtimes',afun,atype,afcnstr,x,varargin{:});
...
for ii = 1 : maxit
    ...
    if existM1
        y = MYiterappComplex('mldivide',m1fun,m1type,m1fcnstr,r,varargin{:});
        if ~all(isfinite(y))
            flag = 2;
            break
        end
    ...
    if existM2
        z = MYiterappComplex('mldivide',m2fun,m2type,m2fcnstr,y,varargin{:});
        if ~all(isfinite(z))
            flag = 2;
            break
        end
...
%         end
    ...
    q = MYiterappComplex('mtimes',afun,atype,afcnstr,p,varargin{:});
    pq = p.' * q;
%     if ((pq <= 0) || isinf(pq))
%         flag = 4;
%         break
...
%                 if ~warned
%                     warning('MATLAB:pcg:tooSmallTolerance', ...
%                         'Input tol may be smaller than eps*cond(A) and might not be achieved by PCG\n         Try to use a bigger tolerance');
%                 end
%                 flag = 3;
%                 iter = ii;
%                 break;
...
if (flag == 0)
    ...
    r_comp = b - MYiterappComplex('mtimes',afun,atype,afcnstr,xmin,varargin{:});
